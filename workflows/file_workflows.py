...
# as Ctrl+N). Confirmed live: neither an instant teleport-hover
# (move_mouse_input to the item) nor a multi-step gradual mouse move ever
# rendered the fly-out in a captured screenshot, even after a 2.5s dwell.
# The fly-out only actually renders under a real mouse-button-held drag
# (SetCursorPos + MOUSEEVENTF_LEFTDOWN, move while still down, then
# release over the target item) - i.e. the same physical gesture a human
# uses to cascade through a nested menu without letting go of the mouse
# button. This matches Windows' classic "menu tracking" behavior: submenu
# hover-expansion during OS-level menu tracking only responds to messages
# generated by an actual held mouse-down, not synthetic WM_MOUSEMOVE/
# SetCursorPos calls alone while no button is down.
#
# Fly-out item x-offset is `_APP_MENU_ITEM_X_OFFSET` (New's own row) + this
# extra column offset, since the fly-out panel renders to the right of the
# main backstage list.
...
def _click_new_document_flyout_item(app_obj, flyout_index):
    ...
    from "New"'s fly-out submenu (AccuMate Config File / Translation /
    Equation Set / Driver Database - see _NEW_FLYOUT_*_INDEX constants).

    "New" DOES have a real fly-out (confirmed live via a screenshot showing
    "AccuMate Config File", "Translation", "Equation Set", "Driver
    Database"), but it only renders under a genuine held-mouse-button drag
    gesture - a plain click_input() (or even a slow, multi-step
    move_mouse_input hover with no button held) never triggers it and
    instead just runs "New"'s default action (a blank AccuMate Config
    File). See the _APP_MENU_NEW_FLYOUT_* constants' comments for the full
    explanation. This uses win32api SetCursorPos + mouse_event directly
    (not pywinauto's click_input) since pywinauto's synthetic input didn't
    reproduce the real mouse-down-and-drag OS menu-tracking behavior.
    """
    win = app_obj.get_window()
    win.set_focus()
    time.sleep(0.3)
    ...
    new_x = win_rect.left + (btn_rect.left - win_rect.left) + _APP_MENU_ITEM_X_OFFSET
    new_y = win_rect.top + (btn_rect.bottom - win_rect.top) + _APP_MENU_ITEM_Y_OFFSETS[_APP_MENU_NEW_INDEX]

    flyout_x = win_rect.left + (btn_rect.left - win_rect.left) + _APP_MENU_ITEM_X_OFFSET + _APP_MENU_NEW_FLYOUT_X_OFFSET
    flyout_y = win_rect.top + (btn_rect.bottom - win_rect.top) + _APP_MENU_NEW_FLYOUT_Y_OFFSETS[flyout_index]

    win32api.SetCursorPos((new_x, new_y))
    time.sleep(0.3)
    win32api.mouse_event(win32con.MOUSEEVENTF_LEFTDOWN, 0, 0, 0, 0)
    time.sleep(0.5)
    win32api.SetCursorPos((flyout_x, flyout_y))
    time.sleep(0.6)  # let the fly-out panel finish rendering before release
